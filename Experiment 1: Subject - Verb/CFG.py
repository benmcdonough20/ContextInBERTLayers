...
#originally, we thought we would use multiple grammars for comparison
terminals_fish = [
...
def concat(*arrs): #concatenate arrays
    ret = []
    for arr in arrs:
        ret += arr
    return ret

def gen_grammar(terminals): #generate a grammar with the given terminals
...
class Sentence:
    ...
    def get_phrase(self, node, branches): #to retrieve the main noun/verb in the sentence
    ...
    def add_pronoun(self, pronouns): #to replace the subject with a pronoun
...
class Node:
    ...
    def tokenize(self): #recursively tokenize
        if len(self.leaves) == 0:
            return self.tokens
        return concat(*[l.tokenize() for l in self.leaves])

    def first_left_subtree(self): #for getting position of node in tree
    ...
    def token_positions(self): #get position of node tokens in tokenized sentence
...
class EmptyNode: #does not automatically recurse
...
sentences = gen_svsentences(terminals_mammals, 91000)
