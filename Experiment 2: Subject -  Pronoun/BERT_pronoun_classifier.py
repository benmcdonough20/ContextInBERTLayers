...
CONFIG = BertConfig.from_pretrained('bert-base-uncased', output_hidden_states = True) #get intermediate hidden states

BERTHSSIZE = 768
MAXTOKS = 1 #max number of tokens per classifying word -- 1 for pronouns

import json

datafile = open("train_data.json", "r") #generate this by running CFG.py
dataset = json.load(datafile)

classes = list(set([t["class"] for t in dataset['train']])) #list of classes

def dirac_mass(cat): #create a distribution for cross-entropy
  ret = [0]*len(classes)
...
if __name__=='__main__':
  ITER = 4 #which layer to train on
  ...
  for index, example in enumerate(dataset['train']):
    ...
    tokenized_sentence, token_idxs, correct_label = example['input'], example['pron_tok_pos'], example['class'] #get labels and token positions

    ideal_dist = torch.Tensor(dirac_mass(correct_label)).to(device=DEVICE) #dirac mass at correct label
    predicted = cls([token_idxs, tokenized_sentence]) 
    loss += loss_function(predicted, ideal_dist) #compute X-entropy  loss
