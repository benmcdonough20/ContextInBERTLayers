...
class Sentence:
    ...
    def bookend_nouns(self): #want to replace the first and last noun in tree with pronouns!
...
class Node:
    ...
    def find_leaves(self): #return all leaves in subtree
        if len(self.leaves) == 0:
            return [self]
...
def gen_gender_sentences(num):
    ...
    for i in tqdm.tqdm(range(num)):
        ...
        s = Sentence(grammar)
        while noun1.word == noun2.word: #make sure there are at least two distinct nouns
            s = Sentence(grammar)
        ...
        par1 = noun1.parent
        while par1.word != "NP": #follow tree up to nearest NP
            par1 = par1.parent
        par1.leaves = [EmptyNode(name1, grammar)] #replace with name
